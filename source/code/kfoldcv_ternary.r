...
Kcrossvalidation    <- 2                                                                           # number of k-fold sets
boneType            <- "ribs"                                                                      # enter "femur" or "ribs" 
dataFile            <- "c:/dino_nature/data/D10_ribs.csv"               # bone data file (with path)
applyDataFilter     <- FALSE                                                                      # enter FALSE (no filter) or filter name. (see auxiliary.r) 
addTreeTips         <- TRUE                                                                        # FALSE for k-foldcv normally (unless still including test taxa in which new variants have been added)
...
run_kfoldcv <- function(){
  ...
  # drop test taxa (taxa whose diving.or.not is UNKNOWN labels. In cross-validation analysis, we deal with the training part of the data but
  # by separating it into K groups and treating one group as "test" while others kept as training. if want we can add the 
  # actual test taxa later just before the prediction stage (see below) 
  ...
  for (set_no in cross.validation.sets$.id){
