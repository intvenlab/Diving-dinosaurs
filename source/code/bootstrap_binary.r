...
# provide user input
baseDir             <- "c:/dino_nature/output/test_bootstrap"                    # base directory where bootstrap sets are located
Nbootstrapsets      <- 50                                                                      # number of bootstrap sets
boneType           <- "ribs"                                                                  # enter "femur" or "ribs" 
applyDataFilter     <- "Fabbri"                                                                  # enter FALSE (no filter) or filter name. (see auxiliary.r) 
addTreeTips         <- TRUE                                                                     # TRUE for bootstrap data sets have replaced taxa, need to add to tree tips
...
MC_bootstrap <- function(set_no, baseDir, Nbootstrapsets, phyloFDAfile, auxFile, boneType, applyDataFilter, addTreeTips, Ntrees, calculateTrees, saveTrees){
  print(paste0("******* Working on bootstrap set:", set_no, "********"))
  source(phyloFDAfile)    
  source(auxFile) 
  workDir <- file.path(baseDir, paste0("set_", set_no))
  setwd(workDir)
  regressionData.file <- file.path(workDir, paste0(boneType,"_set_",set_no,".csv"))    # bone data file (with path)
  if (!file.exists(regressionData.file)){
    stop("* ERROR: Data file for regression does not exist! Check the path and the data (see also README in main directory for conventions.)")
  }
  ...
  cat("Logfile for a session of program: bootstrap_binary.r", sep="\n", file=logFile)
  cat("-----------------------------------------------------", sep='\n',file=logFile, append = T)
  cat("INPUT:", sep="\n", file=logFile, append = T)
  cat(paste0("workDir: ", workDir), sep="\n", file=logFile, append = T)
  cat(paste0("Nbootstrapsets: ", Nbootstrapsets), sep="\n", file=logFile, append = T)
  cat(paste0("phyloFDAfile: ", phyloFDAfile), sep="\n", file=logFile, append = T)
  cat(paste0("auxFile: ", auxFile), sep="\n", file=logFile, append = T)
  cat(paste0("boneType: ", boneType), sep="\n", file=logFile, append = T)
  cat(paste0("regressionData.file: ", regressionData.file), sep="\n", file=logFile, append = T)
  ...
  if (addTreeTips){
    data.histo <- read.csv(regressionData.file, row.names = 1)
  ...
  # before random scaled trees are generated we need to deal with the issue of duplicate taxa 
  if (addTreeTips){
...
run_bootstrap <- function(){
  ...
  #system.time(bootstrap_output <- lapply(1:Nbootstrapsets, MC_bootstrap, baseDir=baseDir, Nbootstrapsets=Nbootstrapsets, phyloFDAfile=phyloFDAfile, auxFile=auxFile, boneType=boneType, applyDataFilter=applyDataFilter, addTreeTips=addTreeTips, Ntrees=Ntrees, calculateTrees=calculateTrees, saveTrees=saveTrees))
  ...
  bootstrap_output <- parLapply(cl, 1:Nbootstrapsets, MC_bootstrap, baseDir=baseDir, Nbootstrapsets=Nbootstrapsets, phyloFDAfile=phyloFDAfile, auxFile=auxFile, boneType=boneType, applyDataFilter=applyDataFilter, addTreeTips=addTreeTips, Ntrees=Ntrees, calculateTrees=calculateTrees, saveTrees=saveTrees)
  stopCluster(cl)
}


if (runStandAlone){
  result <- system.time(run_bootstrap())
