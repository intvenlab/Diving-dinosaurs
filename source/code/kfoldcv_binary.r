#   kfoldcv_binary.r
#   K-fold cross-validation code suitable for binary classification
#   Cem Ozen, Aug. 13, 2022
#
#   STACKHEAD 8 
...
# provide user input
baseDir             <- "c:/dino_nature/output/test_kfoldcv"                             # base directory where k-fold cross-validation sets will be located
Kcrossvalidation    <- 10                                                                           # number of k-fold sets
...
addTreeTips         <- FALSE                                                                        # FALSE for k-foldcv normally (unless still including test taxa in which new variants have been added)
...
MC_kfoldcv <- function(set_no, logFile, baseDir, auxFile, phyloFDAfile, Ntrees, cross.validation.sets, data.histo.keep, data.histo.test, scaled.phylos){
  set_no = as.integer(set_no)
  
  message <- paste0("******* Working on K-fold CV set:", set_no, "********")
  print(message)
  ...
  # reset current sets data to our kept data consisting of only training taxa with their genuine labels
  data.histo <- data.histo.keep
  # define test sets and training sets
  test.set <- rownames(as.data.frame(cross.validation.sets$test[set_no]))     
  # make the set test a test set indeed, so set diving.or.not to UNKNOWN 
  data.histo[test.set, "diving.or.not"] <- "UNKNOWN"
  
  # we can just make a prediction for the current part of K-fold partition. But we can also still predict the real test taxa that
...
run_kfoldcv <- function(){
  ...
  cat("Logfile for a session of program: kfoldcv_binary.r", sep="\n", file=logFile)
  cat("-----------------------------------------------------", sep='\n',file=logFile, append = T)
  cat("", sep="\n", file=logFile, append = T)
  ...
  cat(paste0("Kcrossvalidation: ", Kcrossvalidation), sep="\n", file=logFile, append = T)
  ...
  # drop test taxa (taxa whose diving.or.not is UNKNOWN labels. In cross-validation analysis, we don't care about them 
  data.histo <- data.histo[data.histo$diving.or.not != "UNKNOWN",]
  # drop any taxon that is not in original.phylo or trees derived thereof. When a taxon is in training role, if it is not in the tree,
  # it will be dropped from training set automatically. If such a taxon is in "test" role as cross-validation will eventually do, 
  # an error will be thrown since it is not present in the trees
  ...
  # shuffle data before creating cross-validation sets
  data.histo <- data.histo[sample(nrow(data.histo)),]      # shuffle before creating the cross-validation
  # Before the cross-validation starts we will keep a copy of the original dataset, which is training in essence, so we know all real labels.
  # we can use this to compare to labels predicted later. we need it for restoring true labels for taxa
  # switching to training again
  data.histo.keep <- data.histo
  
  # NOW START K-FOLD CROSS-VALIDATION. 
  # First create the training and test set combinations:
  cross.validation.sets <- crossv_kfold(data.histo, k=Kcrossvalidation)   # create the cross-validation training and test sets
  
  #------------- Monte Carlo runs in series -------------------
  
  #system.time(kfoldcv_output <- lapply(cross.validation.sets$.id, MC_kfoldcv, logFile=logFile, baseDir=baseDir, auxFile=auxFile, phyloFDAfile=phyloFDAfile, Ntrees=Ntrees, cross.validation.sets=cross.validation.sets, data.histo.keep=data.histo.keep, scaled.phylos=scaled.phylos)))
 ...
 # clusterEvalQ(cl, library(WGCNA))
  ...
  clusterEvalQ(cl, library(modelr))
  
  
  kfoldcv_output <- parLapply(cl, cross.validation.sets$.id, MC_kfoldcv, logFile=logFile, baseDir=baseDir, auxFile=auxFile, phyloFDAfile=phyloFDAfile, Ntrees=Ntrees, cross.validation.sets=cross.validation.sets, data.histo.keep=data.histo.keep, data.histo.test=data.histo.test, scaled.phylos=scaled.phylos)
  stopCluster(cl)
}


if (runStandAlone){
  result <- system.time(run_kfoldcv())
